#!/usr/bin/env python3
"""
Script to update chat_server.py with the necessary changes to fix the OpenAI integration.
"""

import re

# Read the current chat_server.py file
with open('chat_server.py', 'r') as f:
    content = f.read()

# 1. Replace the OpenAI imports
content = re.sub(
    r'from openai import AsyncOpenAI, OpenAI',
    'from openai import OpenAI',
    content
)

# 2. Remove asyncio import if it's on its own line
content = re.sub(
    r'import asyncio\n',
    '',
    content
...
# 3. Remove redis.asyncio import if it's on its own line
content = re.sub(
...
# 4. Replace the AsyncOpenAI client initialization
client_pattern = r'openai_client = AsyncOpenAI\([^)]*\)'
content = re.sub(
    client_pattern,
...
# 5. Replace the asyncio semaphore
semaphore_pattern = r'openai_semaphore = asyncio\.Semaphore\(OPENAI_CONCURRENCY\)'
content = re.sub(
    semaphore_pattern,
...
# 6 & 7. Remove the async ai_respond and ai_respond_sync functions
# Find the start of the async ai_respond function
ai_respond_start = content.find('async def ai_respond(')
if ai_respond_start != -1:
    # Find the end of the ai_respond_sync function (it comes after ai_respond)
    ai_respond_sync_end = content.find('# Google Calendar setup', ai_respond_start)
    if ai_respond_sync_end != -1:
        # Replace both functions with the new get_ai_response function
        new_function = '''def get_ai_response(convo_id, username, conversation_history, user_message, chat_id, channel, language="en"):
    """
...
'''
        content = content[:ai_respond_start] + new_function + content[ai_respond_sync_end:]

# Write the updated content to the new file
with open('chat_server_new.py', 'w') as f:
    f.write(content)

print("Updated chat_server.py has been created as chat_server_new.py")
print("Review the changes and then rename it to chat_server.py")